...
def test_prediction_reducer():
    ...
    X = np.array([[0.1, 0.7, 0.2, 0.2, 0.5, 0.3], [0.2, 0.5, 0.3, 0.3, 0.3, 0.4], [0.6, 0.2, 0.2, 0.4, 0.4, 0.2]])

    reducer = PredictionReducer(n_models=2, n_classes=3)
    reduced_X = reducer.fit_transform(X)

    # The expected result is a 3x2 matrix
    expected_result = np.array([[0.7 * 1 + 0.2 * 2, 0.5 * 1 + 0.3 * 2], [0.5 * 1 + 0.3 * 2, 0.3 * 1 + 0.4 * 2], [0.2 * 1 + 0.2 * 2, 0.4 * 1 + 0.2 * 2]])
