...
__all__ = ['BaseProcessor', 'display_processor_info', 'CopyPreProcessor', 'FeatureSelectionPreProcessor',
           'TargetSelectionPreProcessor', 'ReduceMemoryProcessor', 'BayesianGMMPreProcessor', 'GroupStatsPreProcessor',
           'TalibFeatureGenerator', 'KatsuFeatureGenerator', 'EraQuantileProcessor', 'AwesomePreProcessor']
...
# Cell
class BayesianGMMPreProcessor(BaseProcessor):
    """
    Generate synthetic data using Bayesian Gaussian Mixture models.
    ...
    def _get_coefs(self, dataf: NumerFrame) -> np.ndarray:
        """
        ...
        """
        coefs = []
        era_list = dataf[dataf.meta.era_col].unique()
        for era in era_list:
            # Get features and target from data and center
            features, target = dataf[dataf.era == era]\
    ...
    def _fit_bgmm(self, coefs: np.ndarray, n_components: int) -> BayesianGaussianMixture:
