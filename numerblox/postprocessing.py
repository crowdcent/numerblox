...
__all__ = ['BasePostProcessor', 'Standardizer', 'MeanEnsembler', 'DonateWeightedEnsembler', 'GeometricMeanEnsembler',
           'FeatureNeutralizer', 'FeaturePenalizer', 'TargetNeutralizer', 'AwesomePostProcessor']
...
# Cell
@typechecked
class TargetNeutralizer(BasePostProcessor):
    """
    Neutralize target against arbitrary categorical and/or numerical features.
    :param final_col_name: Column name to store manipulated or ensembled predictions in.
    """

    def __init__(self, target_col: str, final_col_name: str = None, feature_names: list = None, proportion: float = 1.0, cuda=False):
        self.target_col = target_col
        self.proportion = proportion
        self.final_col_name = final_col_name if final_col_name else f"{target_col}_neutralized_{proportion}"
        super().__init__(final_col_name=final_col_name)
        self.feature_names = feature_names
        self.cuda = cuda
    ...
    def transform(self, dataf: NumerFrame) -> NumerFrame:
        feature_names = self.feature_names if self.feature_names else dataf.feature_cols
        ft = FeatureNeutralizer(feature_names=feature_names, pred_name=self.target_col,
                                proportion=self.proportion, cuda=self.cuda)
        neutralized_dataf = ft.transform(dataf.copy())
        dataf.loc[:, self.final_col_name] = neutralized_dataf[f'{self.target_col}_neutralized_{self.proportion}']
        return NumerFrame(dataf)
